...
|--------|--------|--------|------|------|------|
| 消耗 | `Mod属性.消耗` | Integer | 点 | + | MOD容量消耗值 |
...
| 魔力恢复 | `Mod属性.魔力恢复` | Double | 点/s | + | 每秒魔力回复 |
| 魔力上限 | `Mod属性.魔力上限` | Double | 点 | + | 最大魔力值增加 |

**⚠️ 重要说明:** 以上是目前代码中**实际定义**的9个MOD属性(详见`NbtPaths.kt`)。

**扩展属性计划:** 以下属性目前在`mods.yml`配置中有定义，但需要在`NbtPaths.kt`中添加对应常量后才能在代码中使用:
- 护甲、盾牌强度、火焰/冰冻/闪电抗性
- 最大生命、生命吸取、能量回复
- 移动速度、攻击速度、感知范围等

如需使用这些属性，请先在`NbtPaths.kt`的`Mod`对象中添加对应的NBT路径常量。
