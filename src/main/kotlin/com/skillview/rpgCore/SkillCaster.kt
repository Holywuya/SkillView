...
object SkillCaster {
    ...
    private fun executeInternalCast(player: Player, slotIndexStr: String) {
        ...
        val totalEfficiency = getattr(player, "技能效率")
        ...
        val apDmgMore    = getattr(player, "最终伤害")
        val apDmgBonus   = getattr(player, "伤害加成")
        val apExtraRange = getattr(player, "额外范围")
        val apSkillPower = getattr(player, "技能强度")
        ...
        // C. 数值合并并转换
        val realDmgMoreMultiplier     = (apDmgMore + nbtDmgMore).toMultiplier()
        val realDmgBonusMultiplier    = (apDmgBonus + nbtDmgBonus).toMultiplier()
        val realExtraRangeMultiplier  = (apExtraRange + nbtRangeRaw).toMultiplier()
        val realBaseMultiplierPercent = (setting.multiplierUp * nbtLevel + setting.baseMultiplier + nbtMultiplier).toPercent()
        val realSkillPower            = (apSkillPower + nbtSkillPower).toMultiplier()
        // 同步变量到 MythicMobs
        ...
        if (DEBUG) {
            println("""
                ...
                > 最终伤害 (More): $realDmgMoreMultiplier x (AP:$apDmgMore + NBT:$nbtDmgMore)
                > 伤害加成 (Bonus): $realDmgBonusMultiplier x (AP:$apDmgBonus + NBT:$nbtDmgBonus)
        ...
        if (success) {
            // 计算最终 CD 缩减 (AP总和 + 技能书NBT)
            val apCdReduction = getattr(player, "冷却缩减")
            val nbtCdReduction = item?.getDeepDouble("$ROOT_MODIFIER.冷却缩减", 0.0) ?: 0.0
            val totalCdReduction = (apCdReduction + nbtCdReduction).coerceAtLeast(0.0)

            RpgRuntime.setCooldown(player, skillId, setting.cooldown, totalCdReduction)

            if (DEBUG) {
                println("[Debug] [成功] 释放成功 | 最终CD缩减: $totalCdReduction% (AP:$apCdReduction + NBT:$nbtCdReduction)")
            }
        } else {
