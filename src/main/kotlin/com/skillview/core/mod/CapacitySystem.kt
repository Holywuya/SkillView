...
object CapacitySystem {
    ...
    fun calculateUsedCapacity(
        inventory: Inventory,
        modSlots: List<Int>,
        slotPolarities: Map<Int, String> = emptyMap()
    ): Int {
        var total = 0
        modSlots.forEachIndexed { index, slot ->
            val item = inventory.getItem(slot)
            if (item != null && !item.isAir()) {
                val baseCost = item.getModCost()
                val modPolarity = item.getModPolarity()
                val slotPolarity = slotPolarities[index] ?: "无"
                total += PolaritySystem.calculateModCost(modPolarity, slotPolarity, baseCost)
            }
        }
        return total
    }

    fun calculateUsedCapacity(mods: Map<Int, ItemStack>, slotPolarities: Map<Int, String> = emptyMap()): Int {
        var total = 0
    ...
    fun validateLoadout(
        ...
        slotPolarities: Map<Int, String>,
        maxCapacity: Int
    ): CapacityResult {
        val used = calculateUsedCapacity(mods, slotPolarities)
